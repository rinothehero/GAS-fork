...
def backup_bn_stats(model):
    ...
    for name, module in model.named_modules():
        if isinstance(module, (nn.BatchNorm1d, nn.BatchNorm2d, nn.BatchNorm3d)):
            bn_stats[name] = (
                module.running_mean.clone().cpu() if module.running_mean is not None else None,
                module.running_var.clone().cpu() if module.running_var is not None else None
            )
    return bn_stats
...
def compute_oracle_gradients(user_model, server_model, train_loader, criterion, device,
                              max_batches=None):
    """
    ...
    for images, labels in train_loader:
        ...
        # Accumulate client gradients (USFL-style: no batch_size multiplication needed with sum)
    ...
    # USFL-style: divide by total_samples to get average gradient
    if total_samples > 0:
...
def compute_g_score(oracle_grads, current_grads, return_details=False):
    ...
    if oracle_grads is None or current_grads is None:
        if return_details:
            return {'oracle_norm': float('nan'), 'current_norm': float('nan'), 
                    'G': float('nan'), 'G_rel': float('nan')}
        return float('nan')
    ...
    G_rel = (G / oracle_norm * 100) if oracle_norm > 1e-10 else float('nan')  # Percentage
    
    if return_details:
        return {'oracle_norm': oracle_norm, 'current_norm': current_norm, 'G': G, 'G_rel': G_rel}
    return G
...
class GMeasurementManager:
    ...
    def measure_and_record(self, user_model, server_model, split_output, loss, epoch):
        ...
        current_grads = collect_current_gradients(user_model, server_model, split_output, loss)
