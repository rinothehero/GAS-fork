        ...
        if cifar or cinic or cifar100 or SVHN:
            if resnet:
                if split_layer is not None:
                    ...
                    user_model = ResNet18FlexibleClient(split_layer=split_layer)
                    server_model = ResNet18FlexibleServer(split_layer=split_layer, num_classes=num_classes)
                    if twoLogit:
                        server_local_model = ResNet18FlexibleServer(split_layer=split_layer, num_classes=num_classes)
                    ...
                    if twoLogit:
                        server_local_model = ResNet18UpCifarHeavy(num_classes=num_classes)
...
def parse_split_layer(split_layer):
    """
    ...
    result = {'layer': None, 'block': None, 'sublayer': None}
...
class ResNet18FlexibleServer(nn.Module):
    ...
    def _build_partial_block_completer(self):
        ...
        if split_sublayer in ['conv1', 'bn1', 'relu1']:
            ...
            self.partial_block['conv2'] = nn.Conv2d(out_channels, out_channels, kernel_size=3, stride=1, padding=1, bias=False)
